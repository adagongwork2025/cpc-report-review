...
def consolidate_confined_space_logs(logs):
    ...
    - confined_person：同一個人 ID 只保留一筆
    - confined_space：同一個物件只保留一筆
    - heartbeat：同一個人 ID 只保留一筆
    - harness：同一個人 ID 只保留一筆
    """
    if not logs:
        return logs
    ...
    if space_logs:
        ...
        for log in space_logs:
            mesg = log.get('mesg', '')
            # 物件名稱通常在 mesg 中，例如 "偵測到 XXX 物件"
            # 使用整個 mesg 作為物件識別（去除時間和 ID 部分）
    ...
    # 處理 heartbeat：同一個人 ID 只保留一筆
    if heartbeat_logs:
        heartbeat_logs.sort(key=lambda x: x.get('time', ''))
        seen_heartbeat_ids = set()
        for log in heartbeat_logs:
            ...
            if person_id and person_id in seen_heartbeat_ids:
                continue  # 跳過重複的人員 ID
            if person_id:
                seen_heartbeat_ids.add(person_id)
            consolidated.append(log)

    # 處理 harness：同一個人 ID 只保留一筆
    if harness_logs:
        harness_logs.sort(key=lambda x: x.get('time', ''))
        seen_harness_ids = set()
        for log in harness_logs:
            mesg = log.get('mesg', '')
            # 從 mesg 中提取人員 ID
            match = re.search(r'ID[:\s]*(\d+)', mesg, re.IGNORECASE)
            person_id = match.group(1) if match else ''

            if person_id and person_id in seen_harness_ids:
                continue  # 跳過重複的人員 ID
            if person_id:
                seen_harness_ids.add(person_id)
            consolidated.append(log)
...
def generate_review_html(category, date_str, total, cards_html, data_json, report_path, preset_times=None):
    """產生審核工具 HTML
...
{cards_html}
    ...
    <script>
        ...
        function saveState() {{
            const state = {{
                confirmed: Array.from(confirmedIds),
        ...
        function loadState() {{
            ...
            if (saved) {{
                ...
                // 載入確認狀態
                if (state.confirmed && state.confirmed.length > 0) {{
            ...
            // 如果還是沒有，嘗試從報告資料恢復
            if (!stateLoaded) {{
                const reportKey = 'reportData_' + DATE + '_' + CATEGORY;
                const reportData = JSON.parse(localStorage.getItem(reportKey) || '{{}}');
                if (reportData.items && reportData.items.length > 0) {{
                    // 用時間+訊息建立已確認項目的 key
                    const confirmedKeys = new Set(reportData.items.map(item =>
                        item.time_display + '|' + item.mesg
                    ));

                    // 標記已確認的項目，其餘標記為已刪除
                    DATA.forEach(d => {{
                        const key = d.time_display + '|' + d.mesg;
                    ...
                    // 儲存恢復的狀態
                    saveState();
                    console.log('已從報告資料恢復 ' + confirmedIds.size + ' 筆已確認項目');
                }}
            }}
        ...
        function proceedToReport() {{
            const confirmedData = DATA.filter(d => confirmedIds.has(d.id)).map(d => {{
                return {{ ...d, note: notes[d.id] || '' }};
            }});

            // 儲存確認的資料到 localStorage（含備註）
            const reportData = {{
                date: DATE,
                category: CATEGORY,
                items: confirmedData
...
def generate_report_html(category, date_str):
    """產生報告頁面 HTML"""
    return f'''<!DOCTYPE html>
<html lang="zh-TW">
<head>
...
<body>
    ...
    <script>
        ...
        function loadData() {{
            const key = 'reportData_' + DATE + '_' + CATEGORY;
...
<body>
<button class="print-btn" onclick="window.print()">列印 / 存成 PDF</button>`;
...
def generate_manager_report_html(category, date_str):
    """產生管理版報告頁面 HTML（從 localStorage 讀取已確認違規）"""
    return f'''<!DOCTYPE html>
<html lang="zh-TW">
<head>
    <meta charset="UTF-8">
...
def main():
    ...
    for category, logs in categorized.items():
        ...
        # 產生管理版報告頁面（從 localStorage 讀取已確認違規）
