...
# 按時間排序（新到舊）
confirmed_items.sort(key=lambda x: x['time_display'], reverse=True)
...
# 產生卡片 HTML
cards_html = '\n'.join([generate_card_html(item) for item in confirmed_items])
data_json = json.dumps(confirmed_items, ensure_ascii=False)
...
html = re.sub(r'id="confirmed-count">0</span>', 'id="confirmed-count">11</span>', html)
...
# 設定 confirmedIds（全部已確認）
confirmed_ids_str = ','.join(str(item['id']) for item in confirmed_items)
...
# 寫入檔案
output_file = '/Users/ada/Documents/C.客製化/3.中油/2026/需通報列表/cpc通報查詢/2026/04/10/高處B_審核.html'
with open(output_file, 'w', encoding='utf-8') as f:
    f.write(html)

print(f"✓ 已產生審核頁面（只顯示 {len(confirmed_items)} 筆確認違規）")
print(f"  時間範圍: {confirmed_items[0]['time_display']} → {confirmed_items[-1]['time_display']}")
