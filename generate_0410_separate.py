...
print(f"837 第一筆: ID={data_837_processed[0]['id']}, time={data_837_processed[0]['time_display']}")

# 產生卡片 HTML
cards_758 = '\n'.join([generate_card_html(item) for item in data_758_processed])
cards_837 = '\n'.join([generate_card_html(item) for item in data_837_processed])
...
html_a = generate_review_html(
    '高處A', date_str, len(data_837_processed),
    cards_837, json.dumps(data_837_processed, ensure_ascii=False),
    '高處A_報告_管理版.html'
)
...
with open(f'{output_dir}/高處A_審核.html', 'w', encoding='utf-8') as f:
    f.write(html_a)
print(f"✓ 已產生 高處A_審核.html (Camera 837, 從 {data_837_processed[0]['time_display']} 開始)")
