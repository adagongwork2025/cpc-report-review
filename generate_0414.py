#!/usr/bin/env python3
"""
產生 4/14 高處作業審核頁面（Camera 758）
"""
import json
import re

# 讀取 API 資料
with open('/tmp/758_unique_0414.json', 'r') as f:
    ...
    <header class="header">
        <div class="header-inner">
            ...
            <div class="stats-row">
                ...
                <a href="{report_path}" class="btn btn-dark" onclick="saveReportData()">
...
html = generate_review_html(
    '高處作業', date_str, len(data_758_processed),
    cards_758, json.dumps(data_758_processed, ensure_ascii=False),
    '高處作業_報告_管理版.html'
)

# 產生報告頁面
report = generate_manager_report_html('高處作業', date_str)

# 輸出路徑
output_dir = '/Users/ada/Documents/C.客製化/3.中油/2026/需通報列表/cpc通報查詢/2026/04/14'

# 寫入檔案
with open(f'{output_dir}/高處作業_審核.html', 'w', encoding='utf-8') as f:
    f.write(html)
print(f"✓ 已產生 高處作業_審核.html (Camera 758, 從 {data_758_processed[0]['time_display']} 開始)")

with open(f'{output_dir}/高處作業_報告_管理版.html', 'w', encoding='utf-8') as f:
    f.write(report)
print(f"✓ 已產生 高處作業_報告_管理版.html")
